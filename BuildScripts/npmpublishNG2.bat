::================================================
::  This script is used for publishing Angular Framework packages 
::  Do not use for normal application libraries
::================================================
...
call "%SCRIPTDIR%UpgradeVersionNG2\UpgradeVersion.exe" %VERSION%
Echo Setting Version to %VERSION%
...
IF "%SKIPINSTALL%" == "1" GOTO :END 
	...
	echo calling npm cache verify
	ECHO TIMING: npm cache verify START %Time%       
	 IF "0.0.0" == %VERSION%  (	
       call "%SCRIPTDIR%\remove-lock.bat"
    )
    call "%SCRIPTDIR%\verify-cache.bat"       
	ECHO TIMING: npm cache verify END %Time%
