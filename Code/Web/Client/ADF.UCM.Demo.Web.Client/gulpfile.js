...
gulp.task('app-scripts', false, function () {
  ...
  var stream = tsProject
      .src(path.join(paths.clientSrc, '/**/*.ts'))
...
gulp.task('config', false, function () {
  return gulp.src(path.join(paths.clientSrc, '**/config.xml'))
...
gulp.task("copyhtml", ['cachebust'], function () {
  var manifest = gulp.src(paths.clientBuild + "rev-manifest.json");
...
gulp.task('build', false, function (next) {
  $.runSequence('clean', 'copyhtml', next);
});
...
gulp.task('watch', false, ['build'], function () {
  ...
  gulp.watch([path.join(paths.clientSrc, '**/*.ts'), '**/*.tsx'], ['app-scripts'])
      .on('change', onWatchChange);
